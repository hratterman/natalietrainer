...
- `src/lib/booklet/` — the Booklet memorization system (the "400 questions" guide). `types.ts` (client-safe), `canon.ts` (bundles the committed `src/content/booklet.json`; `BOOKLET_FIXTURE=1` and `BOOKLET_PATH` override it), `scheduler.ts` (pure: successive-relearning state machine, deadline-compressed ladder, interleaved queue, pacing projection), `engine.ts` (server glue), `fixture.ts`. Ingest: `npm run booklet:ingest -- <docx>` (`scripts/booklet-ingest.mjs`). The canon is committed and bundled, so no machine needs a setup step; re-run the ingest only when the source guide changes. Tests/e2e run on the original fixture, never the real canon.
